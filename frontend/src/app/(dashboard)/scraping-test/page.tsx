              ...
              <div className="inline-flex p-1 bg-slate-100 rounded-lg border border-slate-200">
                {(['AUTO', 'SCRAPY', 'PLAYWRIGHT'] as const).map((eng) => (
                  <button
                    ...
                    disabled={isRunning}
                    className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${selectedEngine === eng
                ...
                <div className="inline-flex p-1 bg-slate-100 rounded-lg border border-slate-200">
                  {([5, 10, 15] as const).map((limit) => (
                    <button
                      ...
                      disabled={isRunning}
                      className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${resultLimit === limit
                        ? 'bg-white text-blue-700 shadow-2xs'
                        : 'text-slate-600 hover:text-slate-900'
                        }`}
                    >
            ...
            {steps.map((step) => {
              ...
              return (
                <div
                  ...
                  className={`p-2.5 rounded border transition-colors ${isRunningStep
                        ...
                        {testResult.extracted_fares.map((f, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td className="p-2 text-center">
                              <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${f.validation_status.includes('FALLBACK')
