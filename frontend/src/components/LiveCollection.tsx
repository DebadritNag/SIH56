...
export default function LiveCollection() {
  ...
  return <main className="space-y-6 p-6">
    ...
    {run && <section className="space-y-4 rounded border p-4">
      ...
      {run.metadata?.processing && <div className="rounded bg-slate-100 p-4 text-slate-900"><p>Validated: {run.metadata.processing.records_processed} Â· FareGuard: {run.metadata.processing.fareguard_scored} Â· PriceGuard: {run.metadata.processing.priceguard_scored} Â· SHAP: {run.metadata.processing.shap_count}</p><p>APIx: {run.metadata.processing.index.index_value?.toFixed(2) ?? run.metadata.processing.index.status} {run.metadata.processing.index.reason}</p></div>}
    </section>}
  </main>;
}
