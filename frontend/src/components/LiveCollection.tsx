...
export default function LiveCollection() {
  ...
  const config = useQuery({ queryKey: ["live-config", source], queryFn: () => getData<{ server_now?: string; cooldown_until?: string | null; enabled: boolean; worker_enabled: boolean; browser_available?: boolean | null; engine?: string; browser_message?: string; message: string }>("/live/config", { source }), refetchInterval: 15000 });
  ...
  useEffect(() => {
    if (run?.status === "FAILED" || run?.status === "COMPLETED") void cache.invalidateQueries({ queryKey: ["live-config"] });
  }, [run?.status, run?.id, cache]);
  const cooldownSeconds = config.data?.cooldown_until && config.data.server_now
    ? Math.max(0, Math.ceil((Date.parse(config.data.cooldown_until) - Date.parse(config.data.server_now) - (now - config.dataUpdatedAt)) / 1000)) : 0;
  ...
  return <main className="space-y-6 p-6">
    ...
    <label className="flex items-center gap-3">Source<select className={inputClass} value={source} onChange={e => { setSource(e.target.value); collect.reset(); }}><option value="yatra">Yatra</option><option value="happyfares">HappyFares (prototype)</option></select></label>
    ...
    <form className="flex flex-wrap items-end gap-4 rounded border p-4" onSubmit={e => { e.preventDefault(); if (!busy && !cooldownSeconds) collect.mutate(); }}>
