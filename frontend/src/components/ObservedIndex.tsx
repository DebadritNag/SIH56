...
import { APIxReadinessDashboard, type APIxReadiness } from "./readiness/APIxReadinessDashboard";
import { ReadinessSkeleton, ReadinessError } from "./readiness/ReadinessUI";
...
export default function ObservedIndex() {
  ...
  const readiness = useQuery({queryKey:["apix-readiness"], queryFn:()=>getData<APIxReadiness>("/index/readiness"), enabled:!index, refetchInterval:15000});
  ...
  return <main className="space-y-6 p-6"><h1 className="text-2xl font-semibold">Airfare Price Index</h1><p className="text-sm text-slate-500">Persisted national basket calculated from eligible observed fares. Model predictions do not enter APIx.</p>
    ...
    <p className="text-sm text-slate-500">APIx becomes available after genuine history, base-period coverage, and matching route/window data are available.</p>
    {!index && !readiness.data && (latest.isPending || readiness.isPending) && !latest.isError && <ReadinessSkeleton/>}
    {!index && (latest.isError || readiness.isError) && <ReadinessError title="Unable to load APIx readiness" retry={()=>{void latest.refetch();void readiness.refetch();}}/>}
    {!index && readiness.data && <APIxReadinessDashboard data={readiness.data}/>}
